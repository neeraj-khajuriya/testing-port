...
document.addEventListener("DOMContentLoaded", function () {
  const canvas = document.querySelector(".splash-canvas") || document.querySelector("canvas");
  if (!canvas) return;
  ...
  function getWebGLContext(canvasEl) {
    ...
    if (isWebGL2) {
      ...
      halfFloatLinear = glCtx.getExtension("OES_texture_float_linear");
    } else {
      halfFloat = glCtx.getExtension("OES_texture_half_float");
      halfFloatLinear = glCtx.getExtension("OES_texture_half_float_linear");
    }

    glCtx.clearColor(0.05, 0.05, 0.05, 1.0);
    const halfFloatTexType = isWebGL2 ? glCtx.HALF_FLOAT : (halfFloat ? halfFloat.HALF_FLOAT_OES : glCtx.FLOAT);
  ...
  class Material {
    ...
    setKeywords(keywords) {
      ...
      if (!prog) {
        let fragShader = compileShader(gl.FRAGMENT_SHADER, this.fragmentShaderSource, keywords);
        prog = createProgram(this.vertexShader, fragShader);
  ...
  function getUniforms(prog) {
    let uniforms = [];
    let count = gl.getProgramParameter(prog, gl.ACTIVE_UNIFORMS);
    for (let i = 0; i < count; i++) {
      let uniformName = gl.getActiveUniform(prog, i).name;
  ...
  function initFramebuffers() {
    ...
    dye = !dye ? createDoubleFBO(dyeRes.width, dyeRes.height, rgba.internalFormat, rgba.format, texType, filter) :
                 resizeDoubleFBO(dye, dyeRes.width, dyeRes.height, rgba.internalFormat, rgba.format, texType, filter);

    velocity = !velocity ? createDoubleFBO(simRes.width, simRes.height, rg.internalFormat, rg.format, texType, filter) :
  ...
  function initBloomFramebuffers() {
    ...
    bloom = createFBO(res.width, res.height, rgba.internalFormat, rgba.format, texType, filter);
    bloomFramebuffers.length = 0;
    for (let i = 0; i < config.BLOOM_ITERATIONS; i++) {
      let width = res.width >> (i + 1);
      let height = res.height >> (i + 1);
      if (width < 2 || height < 2) break;
      let fbo = createFBO(width, height, rgba.internalFormat, rgba.format, texType, filter);
  ...
  function initSunraysFramebuffers() {
    ...
    sunrays = createFBO(res.width, res.height, r.internalFormat, r.format, texType, filter);
    sunraysTemp = createFBO(res.width, res.height, r.internalFormat, r.format, texType, filter);
  }
  ...
  function resizeFBO(target, w, h, internalFormat, format, type, filter) {
    let newFBO = createFBO(w, h, internalFormat, format, type, filter);
    copyProgram.bind();
  ...
  function drawDisplay(target) {
    ...
    if (config.SUNRAYS) {
      gl.uniform1i(displayMaterial.uniforms.uSunrays, sunrays.attach(2));
    }
    blit(target);
  }
  ...
  function applyBloom(source, destination) {
    ...
    for (let i = 0; i < bloomFramebuffers.length; i++) {
      ...
      gl.uniform2f(bloomBlurProgram.uniforms.texelSize, target.texelSizeX, target.texelSizeY);
      gl.uniform1i(bloomBlurProgram.uniforms.uTexture, target.attach(0));
      blit(dest);
      target = dest;
    }
    ...
    for (let i = bloomFramebuffers.length - 2; i >= 0; i--) {
      let baseTarget = bloomFramebuffers[i];
      gl.uniform2f(bloomBlurProgram.uniforms.texelSize, target.texelSizeX, target.texelSizeY);
      gl.uniform1i(bloomBlurProgram.uniforms.uTexture, target.attach(0));
    ...
    gl.uniform2f(bloomFinalProgram.uniforms.texelSize, target.texelSizeX, target.texelSizeY);
    gl.uniform1i(bloomFinalProgram.uniforms.uTexture, target.attach(0));
